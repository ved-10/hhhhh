var play,about,playimg,aboutimg,back,backImg,aboutbg,g1Img,g2Img,g3Img,g4Img,g5Img,liftImg,plrWR,plrWL,plrShotR,plrShotL,plrst,plrstl;
...
var enyb = createSprite(windowWidth/14,windowHeight/2.4)
enyb.addImage(robo)
enyb.scale = 0.4
var enyhitb = createSprite(enyb.position.x+250,enyb.position.y-18,500,10)
...
var enybBullet = createSprite(enyb.position.x,enyhitb.position.y,10,1)
  ...
  if(enyhitb.isTouching(plr)){
    ...
    if(plr.isTouching(enybBulletGroup)){
        enybBulletGroup.destroyEach()
        hp = hp-1
    }

    enybBullet.lifetime=100;
    enybBulletGroup.add(enybBullet)

var enyc = createSprite(windowWidth-100,windowHeight/2.4)
enyc.addImage(robol)
enyc.scale = 0.4
var enyhitc = createSprite(enyc.position.x-250,enyc.position.y-18,500,10)
...
var enycBullet = createSprite(enyc.position.x,enyhitc.position.y,10,1)
  ...
  if(enyhitc.isTouching(plr)){
    ...
    if(plr.isTouching(enycBulletGroup)){
        enycBulletGroup.destroyEach()
        hp = hp-1
    }

    enycBullet.lifetime=100;
    enycBulletGroup.add(enycBullet)
    ...
    wlsss = [wall18,wall19,wall20,wall21,wall22,wall23,wall24,wall25,wall26,wall27,lift3,lift4]
...
if(keyDown("LEFT_ARROW")||keyDown("a")){
    ...
    plr.changeAnimation("standingL",plrstl)
